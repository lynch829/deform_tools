function varargout = MergeImages(referenceImage, dailyImage, method, bone)
% MergeImages rigidly registers a daily image to a reference image and then
% merges the datasets by resampling the daily image to the reference image
% coordinate system and then using the reference image data outside the
% daily image to create a merged image of the same size and coordinates as
% the reference image.
%
...
%       for options
%   bone: logical indicating whether to mask registration to only bony
%       anatomy (true) or full image (true)
%
...
case 'PM_6DOF_MSE'
    ...
    % Store 4-DOF rigid regsitration results
...
[x,y] = meshgrid(dailyImage.start(2):dailyImage.width(2):...
    dailyImage.start(2) + dailyImage.width(1) * ...
    (dailyImage.dimensions(1) - 1), dailyImage.start(1):...
    dailyImage.width(1):dailyImage.start(1)+...
    dailyImage.width(1) * (dailyImage.dimensions(1) - 1));
